...
2. Craft stage seals 1–10 one at a time. The first ten ingredient materials are, in order,
   `GOLD_INGOT`, `DIAMOND`, `EMERALD`, `AMETHYST_SHARD`, `PRISMARINE_CRYSTALS`, `QUARTZ`,
   `GLOWSTONE_DUST`, `REDSTONE`, `LAPIS_LAZULI`, and `NETHERITE_SCRAP`; each recipe also uses a
   center `NETHER_STAR`.
3. Start stage 2 from the core GUI or by right-clicking a stage-2 seal. Confirm the GUI selects the
...
4. During a wave, identify role names `防衛戦破壊兵`, `防衛戦建築兵`, and the intermediate/final
   boss names. Confirm Destroyers are Husks, Builders are Zombie Villagers, and normal/boss roles
   are Zombies. Confirm every variant can be damaged only by a registered participant or team
   tower and drops no vanilla loot.
